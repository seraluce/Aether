import type { WPPost, WPCategory, WPTag } from "./wordpress";
...
function getTags(post: WPPost): string[] {
  const terms = post._embedded?.["wp:term"];
  if (terms && terms[1]) {
    return terms[1].map((t: { name: string; slug: string }) => t.name);
  }
  return [];
}
...
const svgIcons = [
  ...
  '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14v-4z"/><rect x="3" y="6" width="12" height="12" rx="2"/></svg>',
  ...
  '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 3a3 3 0 00-3 3v12a3 3 0 003 3 3 3 0 003-3 3 3 0 00-3-3H6a3 3 0 00-3 3 3 3 0 003 3 3 3 0 003-3V6a3 3 0 00-3-3 3 3 0 00-3 3 3 3 0 003 3h12a3 3 0 003-3 3 3 0 00-3-3z"/></svg>',
...
function pickIcon(name: string, slug: string): string {
  ...
  return svgIcons[Math.abs(hash) % svgIcons.length];
}

export function mapCategory(cat: WPCategory): {
  name: string;
  slug: string;
  count: number;
  icon: string;
